...
from espnet.nets.pytorch_backend.transformer.truncated_encoder_layer import TruncatedEncoderLayer
...
from espnet.nets.pytorch_backend.transformer.positionwise_feed_forward import (
    TruncatedPositionwiseFeedForward,  # noqa: H301
)
from espnet.nets.pytorch_backend.transformer.repeat import repeat
from espnet.nets.pytorch_backend.transformer.subsampling import Conv2dSubsampling
from espnet.nets.pytorch_backend.transformer.subsampling import Conv2dSubsampling6
from espnet.nets.pytorch_backend.transformer.subsampling import Conv2dSubsampling8
from espnet.nets.pytorch_backend.nets_utils import truncated_op
import pdb
...
class TruncatedEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        super(TruncatedEncoder, self).__init__()
        ...
        self.encoders = repeat(
            num_blocks,
            lambda lnum: TruncatedEncoderLayer(
                attention_dim,
    ...
    def get_positionwise_layer(
        ...
        if positionwise_layer_type == "truncated_linear":
            positionwise_layer = TruncatedPositionwiseFeedForward
            positionwise_layer_args = (attention_dim, linear_units, dropout_rate)
        elif positionwise_layer_type == "truncated_conv1d":
...
#                 dropout_rate,
#             )
            raise NotImplementedError(f"{positionwise_layer_type} is not implemented")
        elif positionwise_layer_type == "truncated_conv1d-linear":
...
#             )
    ...
    def forward(self, xs, masks, chunk_start, chunk_end):
        ...
        if isinstance(
            ...
            # xs = self.embed(xs)
            xs = truncated_op([chunk_start, chunk_end, 1], self.embed, [xs])

        if self.intermediate_layers is None:
            xs, masks = self.encoders(xs, masks, chunk_start, chunk_end)
        else:
            intermediate_outputs = []
            for layer_idx, encoder_layer in enumerate(self.encoders):
                xs, masks = encoder_layer(xs, masks, chunk_start, chunk_end)

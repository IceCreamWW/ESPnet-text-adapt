...
ngpu=2               # The number of gpus ("1" uses cpu, otherwise use gpu).
...
nbpe=10000             # The number of BPE vocabulary.
...
phntoken_list="${token_listdir}"/phn/tokens.pd.txt
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        # shellcheck disable=SC2086
        # ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_text_adapt_train \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                --token_list "${token_list}" \
                --phn_token_list "${phntoken_list}" \
                --non_linguistic_symbols "${nlsyms_txt}" \
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/text,text_phn,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text_phn,text" \
        ...
        <"${asr_stats_dir}/train/text_shape" \
            awk -v N="$(<${phntoken_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/text_phn_shape.phn"
        ...
        <"${asr_stats_dir}/valid/text_shape" \
            awk -v N="$(<${phntoken_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/valid/text_phn_shape.phn"
    fi
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text,text_phn,text "
            _opts+="--train_shape_file ${_split_dir}/speech_shape "
            _opts+="--train_shape_file ${_split_dir}/text_shape.${token_type} "
            _opts+="--train_shape_file ${_split_dir}/text_phn_shape.phn "
            _opts+="--multiple_iterator true "
            ...
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text,text_phn,text "
            _opts+="--train_shape_file ${asr_stats_dir}/train/speech_shape "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_shape.${token_type} "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_phn_shape.phn "
        fi
        ...
        # shellcheck disable=SC2086
            ${python} -m espnet2.bin.asr_text_adapt_train \
                ...
                --valid_shape_file "${asr_stats_dir}/valid/text_phn_shape.phn" \
                --resume true \
